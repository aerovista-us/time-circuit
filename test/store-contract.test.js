...
const themeCss = fs.readFileSync('time-circuit-theme-system.css','utf8');
const player = fs.readFileSync('time-circuit.js','utf8');
const visualUpgrade = fs.readFileSync('time-circuit-visual-upgrade.js','utf8');
const themeSystem = fs.readFileSync('time-circuit-theme-system.js','utf8');
...
assert.match(html, /time-circuit-theme-system\.css/);
assert.match(html, /time-circuit-visual-upgrade\.js/);
assert.match(html, /time-circuit-theme-system\.js/);
...
assert.match(themeSystem, /signalThemes/);
assert.match(themeSystem, /'TC-00': \{ id:'retroflux'/);
assert.match(themeSystem, /'TC-02': \{ id:'bruiser'/);
assert.match(themeSystem, /'TC-06': \{ id:'swamp'/);
assert.match(themeSystem, /dataset\.signalTheme/);
assert.match(themeSystem, /dataset\.timeView/);
assert.match(themeSystem, /theme-color/);
assert.match(themeCss, /data-signal-theme="rebel"/);
assert.match(themeCss, /data-signal-theme="paradox"/);
assert.match(themeCss, /data-signal-theme="fracture"/);
assert.match(themeCss, /--signal-a-rgb/);
assert.match(themeCss, /STATIC COVER \/ TIME CIRCUIT/);
assert.match(themeCss, /\.controls #btnPlay/);
assert.match(themeCss, /\.archive-grid\{grid-template-columns:repeat\(2/);
assert.match(themeCss, /\.product-action:not\(:disabled\)/);
...
console.log('Time Circuit listen + store + visualizer v2 + signal theme contracts: OK');
